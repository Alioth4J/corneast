...
public class ReleaseRequestHandlingStrategy implements RequestHandlingStrategy {
    ...
    public CompletableFuture<ResponseProto.ResponseDTO> handle(RequestProto.RequestDTO requestDTO) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            long result = redissonClients.get(random.nextInt(nodeSize)).getScript().eval(RScript.Mode.READ_WRITE, luaScript, RScript.ReturnType.VALUE, List.of(key));
            if (result == 1) {
                ...
                return responseBuilder.setReleaseRespDTO(failRespBuilder.setKey(key).build()).build();
            }
        }, releaseExecutor);
    }
